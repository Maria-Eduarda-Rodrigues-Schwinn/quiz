...
function selecionarQuestoesAleatorias(questoes, quantidadeQuestoes) { 
  ...
  // Embaralha as perguntas do array questoes
  const questoesAleatorias = questoes.sort(() => Math.random() - 0.5);
  
  // retorna um novo array com 10 perguntas aleatórias
  return questoesAleatorias.slice(0, quantidadeQuestoes); 
}

// Função que exibe a pergunta e retornar a resposta do usuário
function exibirPergunta(pergunta, index) { 
  console.log(`\n${index + 1}: ${pergunta.pergunta}`);
  const respostaUsuario = resposta.question('Digite a resposta (ano): ');
...
// Função que retornar uma mensagem com base na pontuação do jogador
...
function iniciarQuiz() {
  ...
  const nomeJogador = resposta.question('Digite o seu nome: ');
